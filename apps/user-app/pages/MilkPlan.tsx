...
const NewSubscription: React.FC = () => {
    ...
    const [milkPrices, setMilkPrices] = useState<{ buffalo: number; cow: number }>({ buffalo: 90, cow: 60 });

    // --- Effects ---
    useEffect(() => {
        const initData = async () => {
            const userIdStr = localStorage.getItem('taaza_user_id');
            const userEmail = localStorage.getItem('taaza_user_email');
            ...
            try {
                // 0. Load Pricing from API
                const pricingData = await MilkService.getPricing();
                if (pricingData && pricingData.length > 0) {
                    const prices: any = {};
                    pricingData.forEach((item: any) => {
                        prices[item.milk_type] = item.price_per_liter;
                    });
                    setMilkPrices(prices);
                }

                // 1. Load Addresses
                const addrs = await UserService.getAddresses(userEmail);
                if (addrs) {
                    setAddresses(addrs);
                    // Set logic for default
                    const def = addrs.find((a: any) => a.is_default) || addrs[0];
                    if (def) setSelectedAddressId(def.id);
                }

                // 2. Load Subscription
                const subData = await MilkService.getSubscription(userId);
                if (subData && subData.subscription) {
                    const sub = subData.subscription;
                    setIsUpdateFlow(true);
                    setBackPath('/milk/plan');

                    if (sub.address_id) setSelectedAddressId(sub.address_id);
                    setAutoPay(sub.auto_pay);

                    // Map slots
                    let mSlot = { ...morning, enabled: false };
                    let eSlot = { ...evening, enabled: false };

                    if (sub.slots) {
                        sub.slots.forEach((s: any) => {
                            if (s.slot_type === 'morning') {
                                mSlot = {
                                    enabled: s.is_enabled,
                                    expanded: s.is_enabled,
                                    type: s.milk_type,
                                    quantity: s.quantity,
                                    time: s.time_slot,
    ...
    const getSlotWeeklyCost = (slot: SlotConfig) => {
        ...
        return slot.quantity * milkPrices[slot.type] * days;
    };
    ...
    const handleSubscribe = async () => {
        ...
        if (!userIdStr || !userEmail) {
            // Should normally redirect to login
            setToastMsg({ msg: "Session expired. Please login again.", type: 'error' });
        ...
        if (morning.enabled) {
            slots.push({
                slot_type: 'morning',
                milk_type: morning.type,
                quantity: morning.quantity,
                time_slot: morning.time,
                frequency: morning.frequency,
                days: morning.frequency === 'daily' ? [] : morning.days,
                is_enabled: true
            });
        }
        if (evening.enabled) {
            slots.push({
                slot_type: 'evening',
                milk_type: evening.type,
                quantity: evening.quantity,
                time_slot: evening.time,
                frequency: evening.frequency,
                days: evening.frequency === 'daily' ? [] : evening.days,
                is_enabled: true
            });
        }
        ...
        try {
            await MilkService.createSubscription(subscriptionData);
            ...
            setToastMsg({ msg: "Failed to create subscription. Please try again.", type: 'error' });
            setIsSubmitting(false);
        }
    };
    ...
    const saveNewAddress = async () => {
        ...
        const addressData = {
            ...
            full_address: newAddress.mapAddress,
            latitude: 12.9716,
            longitude: 77.5946,
            receiver_name: newAddress.receiverName,
            receiver_phone: newAddress.receiverPhone,
            is_default: newAddress.isDefault
        };

        try {
            const res = await UserService.saveAddress(addressData);
            // Refresh addresses
            const addrs = await UserService.getAddresses(userEmail);
            setAddresses(addrs);
            ...
            if (addressData.id) {
                // Edited
                setSelectedAddressId(addressData.id.toString());
            } else if (res && res.id) {
                // Created
                setSelectedAddressId(res.id.toString());
            }
            ...
            <div className="p-4 space-y-5 animate-slide-up">
                                ...
                                <div>
                                    ...
                                    <div className="flex bg-gray-50 p-1 rounded-xl border border-gray-100">
                                        ...
                                        <button
                                            ...
                                            className={`flex-1 py-2 px-1 rounded-lg text-xs font-bold transition-all ${morning.frequency === 'alternate'
                                ...
                                <div>
                                    ...
                                    <div className="flex bg-gray-50 p-1 rounded-xl border border-gray-100">
                                        ...
                                        <button
                                            ...
                                            className={`flex-1 py-2 px-1 rounded-lg text-xs font-bold transition-all ${evening.frequency === 'alternate'
                ...
                <div className={`bg-white rounded-2xl p-4 shadow-sm border transition-all ${errorFlags.includes('address') ? 'border-red-500 shadow-red-100 ring-1 ring-red-100' : 'border-gray-100'}`}>
                    ...
                    {selectedAddress ? (
                        ...
                        <button
                            onClick={() => setAddressView('list')}
                            className={`w-full py-4 border-2 border-dashed rounded-xl text-sm font-bold transition-colors flex items-center justify-center gap-2 ${errorFlags.includes('address')
...
const ManagePlan: React.FC = () => {
    ...
    const calculateWeeklyTotal = (sub: any) => {
        ...
        if (sub.morning.enabled) {
            ...
            total += sub.morning.quantity * milkPrices[sub.morning.type as MilkType] * days;
        }
        if (sub.evening.enabled) {
            const days = sub.evening.frequency === 'daily' ? 7 : sub.evening.days.length;
            total += sub.evening.quantity * milkPrices[sub.evening.type as MilkType] * days;
        }
        return total;
    };
                ...
                <div className="flex-grow p-6 overflow-y-auto pb-32">
                    ...
                    <div className="space-y-4 animate-slide-up">
                        ...
                        {cancellationReasonsList.map((r) => (
                            <label
                                key={r}
                                className={`flex items-center p-4 rounded-xl border-2 cursor-pointer transition-all duration-200 group ${cancelReason === r
                                    ? 'border-red-500 bg-red-50/30'
                                    : 'border-gray-100 hover:border-gray-200 bg-white'
                                    }`}
                            >
                                <div className={`w-5 h-5 rounded-full border-2 mr-3 flex items-center justify-center transition-colors ${cancelReason === r ? 'border-red-500' : 'border-gray-300 group-hover:border-gray-400'
                                    }`}>
                                            ...
                                            {day.morning && (
                                                <div className={`flex justify-between items-center p-2 rounded-lg ${day.morning.isSkipped ? 'bg-red-50/50' : 'bg-yellow-50/30'}`}>
                                                    ...
                                                    <button
                                                        ...
                                                        disabled={locked}
                                                        className={`text-[10px] font-bold px-3 py-1.5 rounded border transition-all ${locked
