...
const Toast: React.FC<{ message: string; onClose: () => void }> = ({ message, onClose }) => {
    ...
    return (
        <div className="fixed bottom-20 left-1/2 transform -translate-x-1/2 bg-gray-900/90 text-white px-4 py-2.5 rounded-xl shadow-lg z-50 animate-fade-in flex items-center gap-2 backdrop-blur-sm min-w-[200px] justify-center">
...
const PersonalInfo: React.FC = () => {
  ...
  const [image, setImage] = useState<string | null>(localStorage.getItem('taaza_user_image') || null);
  ...
  const handleUpdate = () => {
    ...
    setTimeout(() => {
        ...
        if(mobile) localStorage.setItem('taaza_mobile', mobile);
        if(gender) localStorage.setItem('taaza_user_gender', gender);
        if(dob) localStorage.setItem('taaza_user_dob', dob);
        if(image) localStorage.setItem('taaza_user_image', image);
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="bg-white p-6 flex flex-col items-center border-b border-gray-100">
        <div className="relative group">
          <div className="w-28 h-28 rounded-full bg-gray-100 border-4 border-white shadow-xl overflow-hidden flex items-center justify-center relative">
            {image ? (
          ...
          <label className="absolute bottom-1 right-1 bg-zepto-blue text-white p-2.5 rounded-full cursor-pointer hover:bg-blue-900 shadow-lg border-2 border-white transition-all hover:scale-110 active:scale-95">
            <span className="material-symbols-outlined text-sm block">camera_alt</span>
            <input type="file" className="hidden" accept="image/*" onChange={handleImageChange}/>
          </label>
        </div>
        <p className="text-xs font-medium text-gray-400 mt-3 bg-gray-100 px-3 py-1 rounded-full">Tap camera to upload</p>
      </div>
            ...
            {addresses.map((addr) => (
                <div key={addr.id} className={`bg-white p-5 rounded-2xl shadow-sm border relative group overflow-hidden transition-all ${addr.isDefault ? 'border-green-500 ring-1 ring-green-100' : 'border-gray-100'}`}>
                    ...
                    <div className={`absolute top-0 right-0 px-3 py-1 rounded-bl-xl text-[10px] font-bold uppercase tracking-wider ${
                        addr.tag === 'Home' ? 'bg-blue-100 text-blue-700' : 
                    ...
                    <div className="flex gap-4 mt-2">
                         <div className={`w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center ${
                             addr.tag === 'Home' ? 'bg-blue-50 text-blue-600' : 
...
const MainProfile: React.FC = () => {
  ...
  const userImage = localStorage.getItem('taaza_user_image');
      ...
      <div className="bg-white pb-6 pt-2 rounded-b-3xl shadow-sm mb-6 border-b border-gray-100 relative overflow-hidden">
        ...
        <div className="flex flex-col items-center relative z-10">
          <div className="relative mb-3">
             <div className="w-24 h-24 rounded-full bg-gradient-to-br from-zepto-yellow to-yellow-300 flex items-center justify-center text-4xl font-bold text-zepto-blue shadow-lg border-4 border-white overflow-hidden">
              {userImage ? (
